...
- [x] **AC-1:** Item "Prazos" adicionado no menu de navegação lateral (`/dashboard/prazos`)
- [x] **AC-2:** Tela exibe as 12 etapas conforme planilha "PRAZOS / ETAPAS DE APROVAÇÃO":
...
- [x] **AC-3:** Para cada contrato ativo, exibir badge com a etapa vigente
- [x] **AC-4:** Etapa vigente destacada visualmente (badge indigo com borda colorida)
- [x] **AC-5:** Etapas com prazo vencido exibidas em vermelho com ícone ⚠
- [x] **AC-6:** Link de cada contrato navega para `/dashboard/contracts/[id]/approvals`
- [x] **AC-7:** Viewers veem a tela (read-only — sem ações de edição)
...
- [x] **Task 1:** Adicionar rota `/dashboard/prazos` e item no menu lateral
- [x] **Task 2:** Criar `src/app/dashboard/prazos/page.tsx`
- [x] **Task 3:** Criar componente `PrazosEtapas.tsx` com lista das 12 etapas
- [x] **Task 4:** Buscar contratos ativos e mapear para etapa vigente
- [x] **Task 5:** Implementar destaque visual da etapa vigente + alerta de prazo vencido
...
### Completion Notes:
- ✅ `Sidebar.tsx`: item "Prazos" (Clock icon) adicionado entre Contratos e Oportunidades
- ✅ `PrazosEtapas.tsx`: painel colapsável com 12 etapas, SLA e contagem de contratos por etapa (badge indigo quando há contratos na etapa)
- ✅ `prazos/page.tsx`: fetcha contratos ativos + approval_workflows pending; tabela com Shopping, Vencimento, Etapa Vigente, Prazo da Etapa, Ver Workflow
- ✅ Etapa vigente: badge indigo com borda colorida — destaque visual (AC-4)
- ✅ Prazo vencido: texto vermelho + ícone ⚠ (AC-5); vencimento próximo também em vermelho
- ✅ STEP_LABELS mapeia tanto etapas internas (pre_approval/financial/director/legal) quanto as 12 etapas do cliente
- ✅ npm run lint: PASS
- ✅ npx tsc --noEmit: PASS
- ✅ npm run build: PASS — rota `/dashboard/prazos` confirmada

### Change Log:
- 2026-02-27: Story criada por @architect (Aria) com base nas solicitações do cliente
- 2026-02-27: Implementação completa Story 4.2
