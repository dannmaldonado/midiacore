...
- [ ] **AC-1:** KPI "Oportunidades Abertas" substitui o card "Performance" (que era mockado como 94%) e exibe contagem real de oportunidades ativas
- [ ] **AC-2:** Novo KPI "Orçamentos Pendentes" exibe contagem de contratos com campo `pending_quotes` preenchido
- [ ] **AC-3:** O gráfico `OccupancyChart` usa dados reais de contratos ativos por mês (baseado em `start_date` e `end_date`) ao invés de valores mockados
- [ ] **AC-4:** Dashboard carrega contratos e oportunidades em paralelo (`Promise.all`)
- [ ] **AC-5:** Loading states funcionam corretamente para os novos KPIs
- [ ] **AC-6:** `npm run build` e `npm run lint` passam sem erros
...
- [ ] **Task 1: Atualizar fetch do Dashboard**
  - [ ] 1.1 Abrir `src/app/dashboard/page.tsx`
  - [ ] 1.2 Adicionar import e state para `opportunities`
  - [ ] 1.3 Substituir fetch simples por `Promise.all([contracts, opportunities])`
  - [ ] 1.4 Calcular `openOpportunities` e `pendingQuotes` no `useMemo` de stats

- [ ] **Task 2: Atualizar KPI cards**
  - [ ] 2.1 Substituir card "Performance" por "Oportunidades Abertas" com dado real
  - [ ] 2.2 Decidir e documentar se adiciona 5º card "Orçamentos Pendentes" ou adapta layout
  - [ ] 2.3 Usar ícone `TrendingUp` ou `Target` do Lucide para Oportunidades

- [ ] **Task 3: Atualizar OccupancyChart com dados reais**
  - [ ] 3.1 Abrir `src/components/dashboard/OccupancyChart.tsx` e verificar interface de props
  - [ ] 3.2 Implementar cálculo de contratos ativos por mês nos últimos 6 meses
  - [ ] 3.3 Substituir `baseValues` mock por dados calculados
  - [ ] 3.4 Ajustar label do eixo Y se necessário
...
- `src/components/dashboard/OccupancyChart.tsx` — MODIFICADO
