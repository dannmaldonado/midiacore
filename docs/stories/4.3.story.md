...
- [x] **AC-1:** Dashboard exibe seção/tabela "Contratos para Renovar" com contratos cujo `end_date` está nos próximos **60 dias** (janela de alerta de renovação) — separada do card existente "Expirando em 30 dias"
- [x] **AC-2:** Clicar "Renovar" abre modal com opções:
...
- [x] **AC-3:** Tela Prazos (`/dashboard/prazos`) destaca em amarelo contratos com `end_date` ≤ 60 dias e exibe botão "Renovar" nesses registros
- [x] **AC-4:** Ao confirmar renovação, cria novo registro em `approval_workflows` com a etapa de renovação correta e deadline calculado
- [x] **AC-5:** Contrato atualizado com `current_step` = etapa de renovação escolhida
- [x] **AC-6:** Notificação gerada para o usuário responsável pela nova etapa
- [x] **AC-7:** Apenas `admin` pode iniciar renovação; `viewer` vê o status mas não o botão
...
- [x] **Task 1:** Migração: adicionar `current_step` em `contracts`
- [x] **Task 2:** Criar componente `RenovacaoModal.tsx`
- [x] **Task 3:** Criar seção "Contratos para Renovar" no dashboard (filtro 60 dias) com botão "Renovar" por linha
- [x] **Task 4:** Adicionar botão "Renovar" na tela Prazos (AC-3)
- [x] **Task 5:** Lógica de INSERT em `approval_workflows` + UPDATE em `contracts`
- [x] **Task 6:** Integrar notificação (reusar hook `use-notifications.ts`)
- [x] **Task 7:** Controle de acesso (admin only)
...
### Completion Notes:
- ✅ `20260227000000_add_current_step_to_contracts.sql`: ADD COLUMN current_step TEXT a contracts
- ✅ `src/types/index.ts`: current_step?: string | null adicionado ao tipo Contract
- ✅ `RenovacaoModal.tsx`: modal com seleção de tipo (sem/com troca), INSERT approval_workflows, UPDATE contracts.current_step, RPC create_approval_notification
- ✅ `dashboard/page.tsx`: seção "Contratos para Renovar" (60 dias), botão Renovar visível apenas para admin, RenovacaoModal integrado
- ✅ `prazos/page.tsx`: destaque amarelo (bg-amber-50/40) para contratos ≤ 60 dias, botão "Renovar" na coluna Ações (admin only), fetchData exposto para refresh pós-renovação
- ✅ npm run lint: PASS
- ✅ npx tsc --noEmit: PASS
- ✅ npm run build: PASS
...
- 2026-02-27: Implementação completa Story 4.3
